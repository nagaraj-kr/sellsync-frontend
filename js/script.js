...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // =========================
    // 2. REGISTRATION MODULE (No changes here)
    // =========================
    const registerForm = document.getElementById("signupForm");
    const roleButtons = document.querySelectorAll(".role-button");
    ...
    if (loginForm) {
        loginForm.addEventListener("submit", async (e) => {
            ...
            try {
                ...
                if (!response.ok) {
                    // 401 Unauthorized varum podhu inga catch aagum
                    throw new Error("Invalid Email or Password");
                }

                const data = await response.json();
                
                // Backend role "ADMIN" or "WHOLESALER" nu varum, safe-ah uppercase check pannuvom
                const role = data.role ? data.role.toUpperCase() : ""; 

                Swal.fire({
                    icon: "success",  // Mismatch error-ah fix panna simple string
                    title: "Login Success",
                    ...
                    if (role === "ADMIN") {
                        ...
                        Swal.fire("Error", "Role not recognized: " + role, "error");
                    }
                });

            } catch (error) {
                console.error("LOGIN ERROR:", error);
                // Authentication failed aana inga dhaan varum
                Swal.fire("Error", "Login Failed: Check your credentials or approval status.", "error");
            }
        });
    }
});
